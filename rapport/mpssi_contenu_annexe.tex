...
\begin{enumerate}[leftmargin=*]
    ...
    [\%\{TIMESTAMP\_ISO8601:log\_time\}] \textbar{} \%\{WORD:log\_level\} \textbar{} IP: \%\{IP:client\_ip\} \textbar{} METHOD: \%\{WORD:http\_method\} \textbar{} PATH: \%\{URIPATH:request\_path\} \textbar{} STATUS: \%\{NUMBER:http\_status:int\} \textbar{} DURATION: \%\{NUMBER:latency\_ms:float\}ms
    \end{tcolorbox}
...
Le dispositif d'audit de sécurité s'articule en deux étapes : \textbf{(1)} l'audit de transport frontal évaluant la terminaison TLS 1.3 et le rejet mTLS strict (HTTP 400) au niveau de NGINX sur le port 443 ; \textbf{(2)} l'audit dynamique applicatif DAST par OWASP ZAP (version 2.14.0 exécutée sous conteneur Docker via \texttt{zap-api-scan.py} ciblant \texttt{http://banking\_backend:8000/openapi.json}, durée 272~s). Ce scan applicatif direct permet d'éprouver la robustesse intrinsèque des 10 endpoints bancaires sans masquage par le régulateur de débit frontal, selon la matrice détaillée dans le Tableau~\ref{tab:annexe_zap_matrix}.
...
\subsection*{C.3 Synthèse du rapport brut OWASP ZAP (Scan Actif d'API)}
Le scan actif complet exécuté par la commande officielle \texttt{zap-api-scan.py -t http://banking\_backend:8000/openapi.json -f openapi -r zap\_report.html} a importé la définition OpenAPI/Swagger de l'API, exploré les 10 endpoints bancaires et injecté des vecteurs d'attaque active (injections SQL/NoSQL, tests BOLA/BFLA, manipulation de paramètres et contournements). Le scan a émis 128 requêtes dynamiques et validé un score global satisfaisant (0 High, 0 Medium, 2 Low, 4 Informational). Le rapport complet est archivé dans \texttt{documentation/zap\_report.html}.
